...
def getWeather():
    if area == "NO":
        ...
        location_id = json.loads(requests.get(url='https://geoapi.qweather.com/v2/city/lookup?lang=zh&key='+os.environ.get("QWEATHER")+'&location='+ area, headers=hea).text)['location'][0]['id']
        url = 'https://devapi.qweather.com/v7/weather/now?lang=zh&location='+location_id + '&key=' + os.environ.get("QWEATHER")
